<div class="container">

<!-- ERP Apps-->
<h3>ERP</h3>
<ul class="thumbnails">
  <li class="span2">
    <a href="<?php echo site_url("inventory");?>" class="thumbnail">
      <img src=<?php echo base_url("resources/images/icons/inventory.png")?> alt="Inventory">
	  <h4><center>Inventory</center></h4>
	</a>
  </li>
  <li class="span2">
    <a href="<?php echo site_url("contacts");?>" class="thumbnail">
      <img src=<?php echo base_url("resources/images/icons/contact.png")?> alt="Contacts">
    <h4><center>Contacts</center></h4>
	</a>
  </li>
  <li class="span2">
    <a href="<?php echo site_url("pos");?>" class="thumbnail">
      <img src=<?php echo base_url("resources/images/icons/POS.png")?> alt="POS">
	  <h4><center>POS</center></h4>
    </a>
  </li>
  <li class="span2">
    <a href="<?php echo site_url("sales");?>" class="thumbnail">
      <img src=<?php echo base_url("resources/images/icons/sales&purchase.png")?> alt="sales&purchase">
	  <h4><center>Sales&Purchase</center></h4>
    </a>
  </li>  
</ul>

<hr/>

<!-- Marketing Apps-->
<h3>Marketing</h3>
<ul class="thumbnails">
  <li class="span2">
    <a href="#" class="thumbnail">
      <img src=<?php echo base_url("resources/images/applogo.png")?> alt="website builder">
    </a>
  </li>
  <li class="span2">
    <a href="#" class="thumbnail">
      <img src=<?php echo base_url("resources/images/applogo.png")?> alt="Facebook">
    </a>
  </li>
  <li class="span2">
    <a href="#" class="thumbnail">
      <img src=<?php echo base_url("resources/images/applogo.png")?> alt="Twitter">
    </a>
  </li>
  <li class="span2">
    <a href="#" class="thumbnail">
      <img src=<?php echo base_url("resources/images/applogo.png")?> alt="Promotion & News Pusher">
    </a>
  </li>  
</ul> 

<hr/>

<!-- Office Apps-->
<h3>Office</h3>
<ul class="thumbnails">
  <li class="span2">
    <a href="#" class="thumbnail">
      <img src=<?php echo base_url("resources/images/applogo.png")?> alt="">
    </a>
  </li>
  <li class="span2">
    <a href="#" class="thumbnail">
      <img src=<?php echo base_url("resources/images/applogo.png")?> alt="">
    </a>
  </li>
  <li class="span2">
    <a href="#" class="thumbnail">
      <img src=<?php echo base_url("resources/images/applogo.png")?> alt="">
    </a>
  </li>
</ul> 