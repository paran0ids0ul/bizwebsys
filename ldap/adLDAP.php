...
class adLDAP {
    ...
    * @var array
    */
    protected $domainControllers = array("bizwebsys.tk");
    ...
    */
    public function connect() {
		...
		$ldap_uri = 'ldap://bizwebsys.tk';
		$ldap_rdn = 'cn=admin,dc=bizwebsys,dc=tk';
		$ldap_pass = 'Abcd1234*';

		$ldapc = ldap_connect($ldap_uri) or die('LDAP connection failed');

		if ($ldapc) {

			return ldap_bind($ldapc, $ldap_rdn, $ldap_pass);
  ...
  /*  public function connect() 
    ...
    */    
    public function adldap_schema($attributes){
        ...
        if (isset($attributes["address_city"]) && !is_null($attributes["address_city"])){ $mod["l"][0]=$attributes["address_city"]; }
        if (isset($attributes["address_code"]) && !is_null($attributes["address_code"])){ $mod["postalCode"][0]=$attributes["address_code"]; }
        //if ($attributes["address_country"]){ $mod["countryCode"][0]=$attributes["address_country"]; } // use country codes?
        if (isset($attributes["address_country"]) && !is_null($attributes["address_country"])){ $mod["c"][0]=$attributes["address_country"]; }
        if (isset($attributes["address_pobox"]) && !is_null($attributes["address_pobox"])){ $mod["postOfficeBox"][0]=$attributes["address_pobox"]; }
        if (isset($attributes["address_state"]) && !is_null($attributes["address_state"])){ $mod["st"][0]=$attributes["address_state"]; }
        if (isset($attributes["address_street"]) && !is_null($attributes["address_street"])){ $mod["streetAddress"][0]=$attributes["address_street"]; }
        if (isset($attributes["company"]) && !is_null($attributes["company"])){ $mod["company"][0]=$attributes["company"]; }
        if (isset($attributes["change_password"]) && !is_null($attributes["change_password"])){ $mod["pwdLastSet"][0]=0; }
        if (isset($attributes["department"]) && !is_null($attributes["department"])){ $mod["department"][0]=$attributes["department"]; }
        if (isset($attributes["description"]) && !is_null($attributes["description"])){ $mod["description"][0]=$attributes["description"]; }
        if (isset($attributes["display_name"]) && !is_null($attributes["display_name"])){ $mod["displayName"][0]=$attributes["display_name"]; }
        if (isset($attributes["email"]) && !is_null($attributes["email"])){ $mod["mail"][0]=$attributes["email"]; }
        if (isset($attributes["expires"]) && !is_null($attributes["expires"])){ $mod["accountExpires"][0]=$attributes["expires"]; } //unix epoch format?
        if (isset($attributes["firstname"]) && !is_null($attributes["firstname"])){ $mod["givenName"][0]=$attributes["firstname"]; }
        if (isset($attributes["home_directory"]) && !is_null($attributes["home_directory"])){ $mod["homeDirectory"][0]=$attributes["home_directory"]; }
        if (isset($attributes["home_drive"]) && !is_null($attributes["home_drive"])){ $mod["homeDrive"][0]=$attributes["home_drive"]; }
        if (isset($attributes["initials"]) && !is_null($attributes["initials"])){ $mod["initials"][0]=$attributes["initials"]; }
        if (isset($attributes["logon_name"]) && !is_null($attributes["logon_name"])){ $mod["userPrincipalName"][0]=$attributes["logon_name"]; }
        if (isset($attributes["manager"]) && !is_null($attributes["manager"])){ $mod["manager"][0]=$attributes["manager"]; }  //UNTESTED ***Use DistinguishedName***
        if (isset($attributes["office"]) && !is_null($attributes["office"])){ $mod["physicalDeliveryOfficeName"][0]=$attributes["office"]; }
        if (isset($attributes["password"]) && !is_null($attributes["password"])){ $mod["unicodePwd"][0]=$this->user()->encodePassword($attributes["password"]); }
        if (isset($attributes["profile_path"]) && !is_null($attributes["profile_path"])){ $mod["profilepath"][0]=$attributes["profile_path"]; }
        if (isset($attributes["script_path"]) && !is_null($attributes["script_path"])){ $mod["scriptPath"][0]=$attributes["script_path"]; }
        if (isset($attributes["surname"]) && !is_null($attributes["surname"])){ $mod["sn"][0]=$attributes["surname"]; }
        if (isset($attributes["title"]) && !is_null($attributes["title"])){ $mod["title"][0]=$attributes["title"]; }
        if (isset($attributes["telephone"]) && !is_null($attributes["telephone"])){ $mod["telephoneNumber"][0]=$attributes["telephone"]; }
        if (isset($attributes["mobile"]) && !is_null($attributes["mobile"])){ $mod["mobile"][0]=$attributes["mobile"]; }
        if (isset($attributes["pager"]) && !is_null($attributes["pager"])){ $mod["pager"][0]=$attributes["pager"]; }
        if (isset($attributes["ipphone"]) && !is_null($attributes["ipphone"])){ $mod["ipphone"][0]=$attributes["ipphone"]; }
        if (isset($attributes["web_page"]) && !is_null($attributes["web_page"])){ $mod["wWWHomePage"][0]=$attributes["web_page"]; }
        if (isset($attributes["fax"]) && !is_null($attributes["fax"])){ $mod["facsimileTelephoneNumber"][0]=$attributes["fax"]; }
        if (isset($attributes["enabled"]) && !is_null($attributes["enabled"])){ $mod["userAccountControl"][0]=$attributes["enabled"]; }
        if (isset($attributes["homephone"]) && !is_null($attributes["homephone"])){ $mod["homephone"][0]=$attributes["homephone"]; }
        
        // Distribution List specific schema
        if (isset($attributes["group_sendpermission"]) && !is_null($attributes["group_sendpermission"])){ $mod["dlMemSubmitPerms"][0]=$attributes["group_sendpermission"]; }
        if (isset($attributes["group_rejectpermission"]) && !is_null($attributes["group_rejectpermission"])){ $mod["dlMemRejectPerms"][0]=$attributes["group_rejectpermission"]; }
        
        // Exchange Schema
        if (isset($attributes["exchange_homemdb"]) && !is_null($attributes["exchange_homemdb"])){ $mod["homeMDB"][0]=$attributes["exchange_homemdb"]; }
        if (isset($attributes["exchange_mailnickname"]) && !is_null($attributes["exchange_mailnickname"])){ $mod["mailNickname"][0]=$attributes["exchange_mailnickname"]; }
        if (isset($attributes["exchange_proxyaddress"]) && !is_null($attributes["exchange_proxyaddress"])){ $mod["proxyAddresses"][0]=$attributes["exchange_proxyaddress"]; }
        if (isset($attributes["exchange_usedefaults"]) && !is_null($attributes["exchange_usedefaults"])){ $mod["mDBUseDefaults"][0]=$attributes["exchange_usedefaults"]; }
        if (isset($attributes["exchange_policyexclude"]) && !is_null($attributes["exchange_policyexclude"])){ $mod["msExchPoliciesExcluded"][0]=$attributes["exchange_policyexclude"]; }
        if (isset($attributes["exchange_policyinclude"]) && !is_null($attributes["exchange_policyinclude"])){ $mod["msExchPoliciesIncluded"][0]=$attributes["exchange_policyinclude"]; }       
        if (isset($attributes["exchange_addressbook"]) && !is_null($attributes["exchange_addressbook"])){ $mod["showInAddressBook"][0]=$attributes["exchange_addressbook"]; }    
        if (isset($attributes["exchange_altrecipient"]) && !is_null($attributes["exchange_altrecipient"])){ $mod["altRecipient"][0]=$attributes["exchange_altrecipient"]; } 
        if (isset($attributes["exchange_deliverandredirect"]) && !is_null($attributes["exchange_deliverandredirect"])){ $mod["deliverAndRedirect"][0]=$attributes["exchange_deliverandredirect"]; }    
        
        // This schema is designed for contacts
        if (isset($attributes["exchange_hidefromlists"]) && !is_null($attributes["exchange_hidefromlists"])){ $mod["msExchHideFromAddressLists"][0]=$attributes["exchange_hidefromlists"]; }
        if (isset($attributes["contact_email"]) && !is_null($attributes["contact_email"])){ $mod["targetAddress"][0]=$attributes["contact_email"]; }
